...
**Minimum cluster resolution.** After fitting, clusters smaller than `min_cluster_floor` (default 50) are absorbed into the per-class noise pseudo-cluster, for every algorithm. Sub-regions below this size cannot support the statistical complexity measures of Step 2 (variance, silhouette, and PCA quantities are undefined or degenerate on a handful of points), so keeping them as standalone analysis units would only produce null or noisy rows.
...
**Threshold semantics.** The default threshold is $\tau = 0$ with a strict inequality: a cluster is *failed* at its first misclassified test sample. This is the most demanding notion of failure — it asks where the classifier is *perfect*, not merely accurate — and is the natural reference point for a diagnostic whose goal is to find any region the classifier cannot fully resolve. Because the binarisation discards the failure magnitude, the full failure-rate distribution of the included clusters is reported alongside the results, so the sensitivity of the conclusions to $\tau$ can be assessed a posteriori without re-running the pipeline.
